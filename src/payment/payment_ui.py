...
class ServiceChargeButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.view.service_charge_btn.label = (
...
class AmountModal(discord.ui.Modal):
    amount_textinput = discord.ui.TextInput(
        label="AMOUNT (3 d.p.)", placeholder="Type the amount here...", required=True
    )
    reason_textinput = discord.ui.TextInput(
        label="REASON", placeholder="Type the reason here...", required=False
    )
    currency_textinput = discord.ui.TextInput(
        label="CURRENCY (JPY, CNY...)", placeholder="HKD", required=False
    )
    ...
    def __init__(self, amount: str, reason: str, currency: str):
        ...
        self.reason = reason
        if (
            reason and self.reason[0] in "(（" and self.reason[-1] in "）)"
        ):  # remove brackets
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(
                B("Invalid amount: Don't divide zero la..."), ephemeral=True
            )
...
class ModalTrigger(discord.ui.Button):
    def __init__(self):
        super().__init__(label="Amount", emoji="💰", row=3, disabled=False)

    async def callback(self, interaction: discord.Interaction):
        self.view.amount_modal = AmountModal(
            self.view.amount_text, self.view.reason, self.view.currency
        )
...
class InputView(discord.ui.View):
    def __init__(
        ...
        self.embed_text = discord.Embed(
            title="Payment record", colour=0xC57CEE, description="??? owe ??? $???"
        )
    ...
    def correct_input(self) -> bool:
        return not (
            self.pay_text == "???"
            or self.paid_text == "???"
            or self.amount_text == "0"
            or self.amount_text == "0.0"
            or self.paid_text in self.pay_text
        )

    def update_description(self) -> None:
        operation = "owe" if self.owe else "pay back"
        currency_text = (
            f" [{self.currency}]" if self.currency != UNIFIED_CURRENCY else ""
        )
        ...
        self.embed_text.description = (
            f"{self.pay_text} {operation} {self.paid_text} "
...
class UndoView(discord.ui.View):
    def __init__(self, show_edit_btn: bool, response_content: str):
        ...
        self.embed_text = discord.Embed(
            title="Payment record successfully updated!", description=response_content
        )

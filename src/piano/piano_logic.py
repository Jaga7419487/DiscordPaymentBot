...
chrome_options.add_argument(
    "--disable-gpu"
)  # Disable GPU (optional, recommended for headless)
chrome_options.add_argument("--no-sandbox")  # Required for some server environments
chrome_options.add_argument(
    "--disable-dev-shm-usage"
)  # Avoids issues in memory-constrained environments
...
def set_path(room: int, day: int, time_slot: int, duration: int) -> list[str]:
    ...
    """
    if not (
        1 <= room <= 2 and 1 <= day <= 7 and 1 <= time_slot <= 30 and 1 <= duration <= 4
    ):
        raise ValueError(
            "[AutoPianoBooking] set_path: Invalid input for booking options"
        )
...
async def piano_system(bot: commands.Bot, message):
    ...
    def check_and_click(xpath: str, timeout=15) -> None:
        present = EC.presence_of_element_located(
            ("xpath", xpath)
        )  # check if button clickable
        WebDriverWait(driver, timeout).until(
            present
        )  # wait until the button is clickable, else return error
    ...
    def check_and_write(xpath: str, text: str, timeout=10) -> None:
        ...
        input_field = driver.find_element(
            "xpath", xpath
        )  # locate the corresponding button
    ...
    try:
        ...
        if not book_now:
            await message.channel.send(
                "**Start waiting for 00:00 to book the piano room!**"
            )

        if await run_blocking(execute_booking):
            await message.channel.send(
                f"**✔ Successfully booked [{menu.embed_text.description}]!**"
            )
        else:
            await message.channel.send(
                f"**❌ Failed to book [{menu.embed_text.description}]!**"
            )
